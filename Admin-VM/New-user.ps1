...
function New-Repositories{
    ...
    foreach ($folder in $folders) {
        try {
            ...
            Write-Log -v_Message "Dossier $folder créé à l'emplacement $folderPath" -v_ConsoleOutput -v_LogLevel "SUCCESS"
        } catch {
...
function Copy-Content{
    ...
    try {
        # Copie les répertoires du répertoire source vers le répertoire utilisateur
...
function Main {
    ...
    if (Test-Path $userPath) {
        Write-Log -v_Message "L'utilisateur $username existe déjà." -v_ConsoleOutput -v_LogLevel "WARN"
